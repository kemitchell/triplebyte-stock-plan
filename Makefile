...
Stock-Plan.docx: Stock-Plan.cform $(CF)
	$(CF) render -f docx -t "[Company Name] [Stock Plan Name]" -n pae $< > $@

Stockholder-Consent.docx: Stockholder-Consent.cform Stockholder-Consent.json $(CF)
	$(CF) render -f docx -t "Action by Written Consent of the Stockholder of [Company Name]" -n rse -s Stockholder-Consent.json $< > $@

Term-Sheet.docx: Term-Sheet.cform $(CF)
	$(CF) render -f docx -t "[Company Name] [Stock Plan Name] Summary of Key Provisions" -n outline $< > $@
