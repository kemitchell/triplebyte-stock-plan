...
.INTERMEDIATE: no-pages.json company-name plan-name

no-pages.json:
	echo '[]' > $@

Stock-Plan.docx: Stock-Plan.cform no-pages.json blanks.json company-name plan-name $(CF)
	$(CF) render $(DOCX) -s no-pages.json -t "$(shell cat company-name) $(shell cat plan-name)" -n outline $< > $@

California-Addendum.docx: California-Addendum.cform no-pages.json blanks.json $(CF)
	$(CF) render $(DOCX) -s no-pages.json -t "California Addendum" -n outline $< > $@

Stockholder-Consent.docx: Stockholder-Consent.cform Stockholder-Consent.json blanks.json company-name $(CF)
	$(CF) render $(DOCX) -t "Action by Written Consent of the Stockholders of $(shell cat company-name)" -n rse -s Stockholder-Consent.json $< > $@

Term-Sheet.docx: Term-Sheet.cform no-pages.json blanks.json company-name plan-name $(CF) $(JSON)
	$(CF) render $(DOCX) -s no-pages.json -t "$(shell cat company-name) $(shell cat plan-name) Summary of Key Provisions" -n outline $< > $@

Option-Notice.docx: Option-Notice.cform Option-Notice.json blanks.json $(CF)
	$(CF) render $(DOCX) -t "Notice of Stock Option Grant" -s Option-Notice.json -n outline $< > $@

Option-Agreement.docx: Option-Agreement.cform no-pages.json blanks.json $(CF)
	$(CF) render $(DOCX) -t "Stock Option Agreement" -s no-pages.json -n ase $< > $@

Country-Addendum.docx: Country-Addendum.cform no-pages.json blanks.json $(CF)
	$(CF) render $(DOCX) -t "Country-Specific Addendum" -s no-pages.json -n outline $< > $@

Option-Exercise-Agreement.docx: Option-Exercise-Agreement.cform Option-Exercise-Agreement.json blanks.json $(CF)
...
company-name: blanks.json $(JSON)
	( $(JSON) 'Company Name' < blanks.json | grep . || echo -n '[Company Name]' ) > $@

plan-name: blanks.json $(JSON)
	( $(JSON) 'Stock Plan Name' < blanks.json | grep . || echo -n '[Stock Plan Name]' ) > $@
...
Early-Exercise-Option-Notice.docx: Early-Exercise-Option-Notice.cform Early-Exercise-Option-Notice.json blanks.json $(CF)
	$(CF) render $(DOCX) -t "Notice of Stock Option Grant" -s Early-Exercise-Option-Notice.json -n outline $< > $@

Early-Exercise-Option-Agreement.docx: Early-Exercise-Option-Agreement.cform no-pages.json blanks.json $(CF)
	$(CF) render $(DOCX) -t "Stock Option Agreement" -n ase -s no-pages.json $< > $@

Early-Exercise-Option-Exercise-Agreement.docx: Early-Exercise-Option-Exercise-Agreement.cform Early-Exercise-Option-Exercise-Agreement.json blanks.json $(CF)
	$(CF) render $(DOCX) -t "Exercise Agreement" -n outline -s Early-Exercise-Option-Exercise-Agreement.json $< > $@

Early-Exercise-Option-Notice-and-Purchase-Agreement.docx: Early-Exercise-Option-Notice-and-Purchase-Agreement.cform Early-Exercise-Option-Notice-and-Purchase-Agreement.json blanks.json $(CF)
...
83b-Election.docx: 83b-Election.cform 83b-Election.json blanks.json $(CF)
	$(CF) render $(DOCX) -t "Election Under Section 83(b) of the Internal Revenue Code of 1986" -n outline -s 83b-Election.json < $< > $@

83b-Statement-Acknowledgment.docx: 83b-Statement-Acknowledgment.cform 83b-Statement-Acknowledgment.json blanks.json $(CF)
...
Board-Consent.docx: Board-Consent.cform Board-Consent.json blanks.json company-name $(CF)
	$(CF) render $(DOCX) -t "Action by Unanimous Written Consent of the Board of Directors of $(shell cat company-name)" -n rse -s Board-Consent.json < $< > $@

Early-Exercise.options:
	echo '{"Early Exercise": true }' > $@

RSPA.docx: RSPA.cform RSPA.json blanks.json $(CF)
	$(CF) render $(DOCX) -t "Restricted Stock Purchase Agreement" -n ase -s RSPA.json < $< > $@

RSA.docx: RSA.cform RSA.json blanks.json $(CF)
